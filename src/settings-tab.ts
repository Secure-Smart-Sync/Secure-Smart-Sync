/**
 * settings-tab.ts
 * Obsidian settings UI for the R2 Sync plugin.
 */
...
export class R2SyncSettingTab extends PluginSettingTab {
  private readonly plugin: R2SyncPlugin;
  ...
  display(): void {
      ...
      .addButton((btn) =>
        btn.setButtonText("Test").onClick(async () => {
          ...
          const ok = await r2.checkConnection((err) => {
            new Notice(`R2 connection failed: ${(err as Error).message}`);
          });
          btn.setDisabled(false);
          btn.setButtonText("Test");
          if (ok) new Notice("✅ R2 connection successful!");
        })
      );
    ...
    new Setting(containerEl)
      ...
      .setDesc("What to do when the same file was changed on both sides.")
      .addDropdown((dd) =>
        dd
    ...
    new Setting(containerEl)
      ...
      .addButton((btn) =>
        btn
          ...
          .setWarning()
          .onClick(async () => {
            (this.plugin as any).resetSyncHistory?.();
          })
      );
  }
}
